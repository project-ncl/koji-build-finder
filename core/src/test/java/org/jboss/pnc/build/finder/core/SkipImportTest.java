...
class SkipImportTest {
    ...
    void verifyMultiImportsKeepEarliest() throws KojiClientException, MalformedURLException {
        ...
        try (KojiClientSession session = new KojiClientSession(config.getKojiHubURL())) {
            ...
            assertThat(
                    builds,
                    hasEntry(is(new BuildSystemInteger(228994L, BuildSystem.koji)), hasProperty("id", is(228994L))));
            assertThat(builds, not(hasKey(new BuildSystemInteger(251444L, BuildSystem.koji))));
        }
    }
}
